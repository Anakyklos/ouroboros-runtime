...
## Phase 1: Setup (Spec Kit + Branch)

**Purpose**: Project initialization — Spec Kit config, branch, verification

- [x] T001 [P] Set up Spec Kit infra (`.specify/`, `.jcode/speckit/commands/` from probe)
- [x] T002 [P] Write project constitution (`.specify/memory/constitution.md`)
...
- [x] T006 [US1] Create `docs/ARCHITECTURE.md` with authoritative architecture doc:
  ...
  - Provenance: Anakyklos/architecture repo not publicly accessible (404); authority derived from #60, #69, #70, and SYSTEM-MAP cited in those issues
- [x] T007 [US1] Rewrite `README.md`:
  ...
  - Add links to docs/ARCHITECTURE.md, docs/LEGACY_MATRIX.md
- [x] T008 [US1] Rewrite `AGENTS.md`:
  ...
  - Add link to docs/ARCHITECTURE.md and docs/LEGACY_MATRIX.md
...
- [x] T017 [US2] Classify `Anti-Vibe workflow` / `PromotionManager` / code-review gates → ADAPT/MOVE (fail-closed gates survive; mission-level verification → Ouroboros; technical software verification → Runstead; promotion/capability evolution → Cadinho)
- [x] T018 [US2] Classify Antigravity bridge / Gemini bridge / Jules bridge → ADAPT/MOVE (connector/capability pattern replaces direct integration; versioned connectors)
...
- [x] T027 [US2] Classify web frontend (Vite/React) → ADAPT/DEFER (componentes reutilizáveis; direção é Mission Control desktop leve; deployment web não obrigatório)
...
- [ ] T032 [US3] Install root dependencies: `bun install --frozen-lockfile`
- [ ] T033 [US3] Install web dependencies: `cd web && bun install --frozen-lockfile && cd ..`
- [ ] T034 [US3] Run full baseline: `bun run check`
- [ ] T035 [US3] Verify whitespace: `git diff --check`
- [ ] T036 [US3] Verify working tree: `git status --short` (only #61 files)
- [ ] T037 [US3] Restore `web/tsconfig.tsbuildinfo` to HEAD (regenerated by tsc -b)
- [ ] T038 [US3] Final search for obsolete concepts as current direction:
...
- [ ] T039 [P] Commit Phase 1 (Spec Kit config + constitution)
- [ ] T040 Commit Phase 2 (docs/ARCHITECTURE.md + docs/LEGACY_MATRIX.md)
- [ ] T041 Commit Phase 3 (README.md rewrite)
- [ ] T042 Commit Phase 4 (AGENTS.md rewrite)
- [ ] T043 Commit Phase 5 (legacy doc markers)
- [ ] T044 Commit Phase 6 (.agent/rules.md update)
- [ ] T045 Commit Phase 7 (validation fixes)
- [ ] T046 Open PR against `main` with `Closes #61`, full description, validation results, follow-ups
