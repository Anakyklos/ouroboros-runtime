...
- [x] T023a [US2] Classify Council UI panels → RETIRE (Council/personas não permanecem arquitetura central; UI futura é projection de Mission facts)
- [x] T023b [US2] Classify Memory UI panel → ADAPT (re-apresentar como Mission state/context projection após #62/#64; não como "agent memory" universal)
- [x] T023c [US2] Classify Terminal UI panels → RETIRE (TUI completa não compete como segunda UI principal; CLI pequena permanece)
- [x] T024 [US2] Classify Electron shell → RETIRE (não-default arquitetural; verificar se existe código)
- [x] T025 [US2] Classify local web server (Fastify/WebSocket) → ADAPT (transporte local versionado #70; IPC local/Unix socket preferido; Fastify mantido até contracts)
