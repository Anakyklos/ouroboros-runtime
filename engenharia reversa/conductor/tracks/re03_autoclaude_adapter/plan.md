...
- [x] Task: Initialize `Auto-Claude/_adapted` as a Bun project (`bun init -y`)
- [x] Task: Configure `tsconfig.json` (strict mode, path aliases)
- [x] Task: Create directory structure:
    - [x] `src/core/domain` (Entities: Agent, Task, Result)
    - [x] `src/core/ports` (Interfaces: Tool, LLM, FileSystem)
    - [x] `src/infrastructure/adapters` (Implementations: BunFile, BunShell, Anthropic)
- [x] Task: Set up testing with `bun:test`
